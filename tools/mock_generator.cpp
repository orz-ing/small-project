...
QVector<User> MockGenerator::generateUsers(int count) {
    ...
    QStringList names = {"张三", "李四", "王五", "赵六", "陈七", "刘八", "孙九", "周十", "吴小明", "郑小红", "Alice", "Bob", "Charlie", "David", "Eva", "Frank", "Grace", "Henry", "Ivy", "Jack", "Kate", "Leo", "Mia", "Nick", "Olivia", "Peter", "Quinn", "Rose", "Sam", "Tina", "Ulysses", "Vera", "Will", "Xena", "Yves", "Zoe", "张伟", "王芳", "李娜", "刘洋", "陈静", "杨磊", "赵敏", "黄婷", "周杰", "吴昊", "徐明", "孙丽", "马超", "朱红", "胡兵", "郭峰"};
...
void MockGenerator::generateToDB(QSqlDatabase db, int bookCount, int userCount,
    ...
    // 保存管理员账号
    auto allExistingUsers = userDao.getAll();
    QVector<User> savedAdmins;
    for (const auto& u : allExistingUsers) {
        if (u.role == UserRole::Admin) {
            savedAdmins.append(u);
        }
    }
    ...
    // 恢复管理员账号
    for (const auto& admin : savedAdmins) {
